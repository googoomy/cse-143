...
def decode(input_length, tagset, score):
    ...
    viterbi = backpointer = numpy.empty(shape=(K,M)) #Define viterbi and backpointer as matrix of size N, T
    ...
    for m in range(2, M):
    	for k in range(0, K):
    		...
    		pass
    #y[M] = argmax(...)
    for m in reversed(range(1, M-1)): #M-1 -> 1
    	#y[m] = backpointer[m] * y[m] + 1
    	pass
    return #y
    ...
    print(tagset)
    #print(score())
    raise NotImplementedError("Complete for part 3")
...
def train(data, feature_names, tagset, epochs):
    ...
    def perceptron_gradient(i):
        ...
        fvector.times_plus_equal(-1, compute_features(gold_labels, input_len,
                                                      features))  # Subtract the features for the gold labels
...
def predict(inputs, input_len, parameters, feature_names, tagset):
    ...
    result = decode(input_len, tagset, score)
    return result
    ...
    with open(out_filename, 'w', encoding='utf-8') as f:
        for inputs in all_inputs:
            ...
            for i, tag in enumerate(tag_seq[1:-1]):  # deletes <START> and <STOP>
                f.write(' '.join([inputs['tokens'][i + 1], inputs['pos'][i + 1], inputs['NP_chunk'][i + 1],
                                  inputs['gold_tags'][i + 1], tag]) + '\n')  # i + 1 because of <START>
            f.write('\n')
...
def evaluate(data, parameters, feature_names, tagset):
    ...
    for inputs in data:
        ...
        all_predicted_tags.extend(
            predict(inputs, input_len, parameters, feature_names, tagset)[1:-1])  # deletes <START> and <STOP>
